...
group.add_argument('--amp', action='store_true', default=True,     #TODO  Default=False
                   help='use NVIDIA Apex AMP or Native AMP for mixed precision training')
group.add_argument('--amp-dtype', default='float16', type=str,
                   help='lower precision AMP dtype (default: float16)')
group.add_argument('--amp-impl', default='native', type=str,
                   help='AMP impl to use, "native" or "apex" (default: native)')
group.add_argument('--no-ddp-bb', action='store_true', default=True, #TODO  Default=False
...
#############################                    END ORIGINAL CODE                 ####################################
#############################                        MY CODE                       ####################################
        if not args.run_wandb:
            args.run_wandb = f"{args.model}_{args.seed}_{args.model_kwargs['initialization']}_{args.epochs}"
...
def train_one_epoch(
    ...
    def capture_gradients(name):
        def hook_fn(grad):
            grad_norm = grad.norm(2).item()  # Compute L2 norm of the gradient
    ...
    return OrderedDict([('loss', losses_m.avg), *eval_metrics.items()])
...
if __name__ == '__main__':
    ...
    #
    # # Start the agent to run the sweep
    # wandb.agent(sweep_id)


    # sweep_configuration = {
    #     'method': 'grid',
    #     'name': 'Compare Initialization Strategy',
    #     'parameters': {
    #         'initialization': {
    #             'values': ['goog', 'custom']
    #         },
    ...
    #         '--warmup-lr', '1e-6',
    #         '--aa', 'v0',
    #         '--seed', '${seed}',  # WandB will replace this dynamically
    #         '--log-wandb',
    #         '--project-wandb', 'COMPARE_INIT_SWEEP',
    #         '--run-wandb', 'SWEEP_${initialization}_INIT_${seed}',  # WandB will replace initialization and seed
    #         '--model-kwargs', 'initialization=${initialization}'  # Dynamic replacement
    #     ]
    # }
    # # Create the sweeper.yaml
    # sweep_id = wandb.sweep(sweep=sweep_configuration, project='COMPARE_INIT_SWEEP')
